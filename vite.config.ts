...
// variable silently becomes `undefined` at runtime. That is exactly how this
// site once shipped a blank white page (the Supabase client threw
// "supabaseUrl is required." before React could mount), so on Vercel a missing
// variable now fails the build instead. Locally it only warns, so `npm run
// build` still works without credentials (CI supplies placeholders).
const REQUIRED_ENV = [
  "VITE_SUPABASE_URL",
...
export default defineConfig(({ mode }) => {
  ...
  const isSet = (key: string) =>
    Boolean((process.env[key] ?? env[key] ?? "").trim());
