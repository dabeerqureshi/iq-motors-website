...
// module load, which prevented React from ever mounting and produced a blank
// white page in production. So when the credentials are missing we log one loud
// actionable error and fall back to a harmless placeholder host instead: the app
// still renders, and the data hooks fall into their normal error/empty states.
...
if (!isSupabaseConfigured) {
  console.error(
    "[IQ Motors] Supabase is not configured: VITE_SUPABASE_URL and/or " +
      ...
      "(Production, Preview and Development) and then redeploy. Inventory, " +
      "vehicle pages and the admin dashboard will stay empty until then."
  );
}
